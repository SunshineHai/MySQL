...
4. ER模型（实体关系模型）和概念模型图。
   ...
   **ER模型**，全称为**实体关系模型**（Entity-Relationship Model），由美籍华裔计算机科学家陈品山先生提出，是概念数据模型的高层描述方式，如下图所示。
   ...
   ![](https://gitee.com/jackfrued/mypic/raw/master/20210731173047.png)
...
1. MySQL中支持多种类型的运算符，我们可以在DQL中使用这些运算符处理数据。

   - 算术运算符（`+`、`-`、`*`、`/`、`%`）
   - 比较运算符（`=`、`<>`、`<=>`、`<`、`<=`、`>`、`>=`、`BETWEEN...AND...`、`IN`、`IS NULL`、`IS NOT NULL`、`LIKE`、`RLIKE`、`REGEXP`）
